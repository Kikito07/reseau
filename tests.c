...
void decode_encode(void)
/*void test_list_fill() {
  int fd = open("testfile.txt", O_CREAT | O_RDWR | O_TRUNC);
  char *buf = "Lorem ipsum dolor sit amet, consectetur adipiscing";
  ssize_t bytes;
  bytes = write(fd, buf, strlen(buf) + 1);
  printf("wrote : %zd\n", bytes);
  list_t *list = init_list();
  int seqn = 0;
  list_fill(list, fd, &seqn);
  CU_ASSERT_STRING_EQUAL(pkt_get_payload(peek(list)), buf);
}*/
int main(void) {
  ...
  pSuite1 =
      CU_add_suite("Test_on_getter_and_setter", init_suite,
                   clean_suite); // creating test suit for getters and setters
  if (NULL == pSuite1) {
    CU_cleanup_registry();
    return CU_get_error();
  }
  ...
  pSuite2 = CU_add_suite("Test_on_varuint", init_suite,
                         clean_suite); // creating suit for varuint
  if (NULL == pSuite2) {
    CU_cleanup_registry();
    return CU_get_error();
  }
