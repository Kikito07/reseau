...
/*
 * Decode un varuint (entier non signe de taille variable  dont le premier bit
 * indique la longueur) encode en network byte-order dans le buffer data
 * disposant d'une taille maximale len.
 * @post: place Ã  l'adresse retval la valeur en host byte-order de l'entier de
 * taille variable stocke dans data si aucune erreur ne s'est produite
 * @return:
 *
 ...
 * @return:
 *           -1 si data ne contient pas une taille suffisante pour encoder le
 * varuint
 *
 *           la taille necessaire pour encoder le varuint (1 ou 2 bytes) si
 ...
 * une fois encodee en varuint si val contient une valeur varuint valide (val <
 0x8000). -1 si val ne contient pas une valeur varuint valide
 */
ssize_t varuint_predict_len(uint16_t val);

/*
 * Retourne la longueur du header en bytes si le champs pkt->length
 * a une valeur valide pour un champs de type varuint (i.e. pkt->length <
 * 0x8000). Retourne -1 sinon
